    ...
    public class RevitFile
    {
        ...
        private void GetAllContents()
        {
            ...
            try
            {
                ...
                {
                    var key = RevitFileUtils.ResolveFileInfoType(streamInfo.Name);
                    if (!SubContents.ContainsKey(key))
                        SubContents.Add(key, StreamUtils.StreamToBytes(streamInfo.GetStream()));
                }
